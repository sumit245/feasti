...
export const setAdrressFromMap = (location) => async (dispatch) => {
    ...
    const city = Array.isArray(address_components) && address_components.filter(item => Array.isArray(item.types) && item.types.some(r => administrative_types.includes(r)))[0].long_name
